...
try
{
    ...
    {
        $search_where .= " AND (
                                q.title LIKE :search_title OR 
                                q.file_path LIKE :search_file_path OR 
    ...
    foreach ($results as &$result)
    {
        if (!empty($auth->getUser()['client_id']))
        {
            $result['edit_url'] = $seoUrl->generate('portal/view_questionnaire.php', ['id' => $result['id']]);
        }
        else if (!empty($auth->getUser()['admin_id']))
        {
            $result['edit_url'] = $seoUrl->generate('administration/view_questionnaire.php', ['id' => $result['id']]);
        }
    }
