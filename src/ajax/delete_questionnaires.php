...
try
{
    ...
    $inData = $db->prepareIn($data['ids'], 'questionnaire_id');
    $fileManager = new FileManager();

    // The admin cannot delete questionnaires created by other
    $count_sql = "SELECT COUNT(q.questionnaire_id) as total 
            FROM questionnaire q
            JOIN appointment a ON q.appointment_id = a.appointment_id
            WHERE q.questionnaire_id IN ({$inData['placeholders']}) 
    ...
    $sql = "SELECT file_path FROM questionnaire WHERE questionnaire_id IN ({$inData['placeholders']})";
    $files = $db->query($sql, $inData['params'])->fetchAll();

    $deleted_count = $db->delete(
        'questionnaire',
        "questionnaire_id IN ({$inData['placeholders']})",
        $inData['params']
    );
