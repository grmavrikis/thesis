...
try
{
    ...
    {
        $search_where = " AND sd.title LIKE :search_term ";
        $search_params = [':search_term' => '%' . $search_term . '%'];
    }

    if (!empty($filters['date_from']))
    {
        $search_where .= " AND s.creation_date >= :date_from ";
        $search_params[':date_from'] = $filters['date_from'] . ' 00:00:00';
    }
    if (!empty($filters['date_to']))
    {
        $search_where .= " AND s.creation_date <= :date_to ";
        $search_params[':date_to'] = $filters['date_to'] . ' 23:59:59';
    }

    if (!empty($filters['tax_id']) && is_array($filters['tax_id']))
    {
        $inData = $db->prepareIn($filters['tax_id'], 'tax_id');
        $search_where .= " AND s.tax_id IN ({$inData['placeholders']}) ";
    ...
    {
        $allowed_sort_columns = ['id', 'title', 'tax_factor', 'sort_order', 'gross_cost', 'creation_date'];
        $allowed_sort_directions = ['ASC', 'DESC'];

        if (in_array($sort_column, $allowed_sort_columns) && in_array(strtoupper($sort_direction), $allowed_sort_directions))
        {
            /* On the left the query select name and on the right the array value from $allowed_sort_columns */
            $sort_column_map = [
                'id' => 's.service_id',
                'title' => 'sd.title',
                'tax_factor' => 't.factor', /* For correct sorting */
                'sort_order' => 's.sort_order',
                'gross_cost' => 's.clean_cost * (1 + t.factor)', /* For correct sorting */
    ...
    $count_sql = "SELECT COUNT(s.service_id) as total 
                  FROM service s 
                  JOIN service_description sd ON s.service_id = sd.service_id 
                  WHERE sd.language_id = :language_id $search_where";
    ...
    $sql = "SELECT 
            s.service_id AS id, 
            sd.title,
            CONCAT(FORMAT(ROUND(t.factor * 100, 2), 2, 'el_GR'), '%') AS tax_factor,
            t.factor, /* For correct sorting */
            s.sort_order, 
            CONCAT(FORMAT(ROUND(s.clean_cost * (1 + t.factor), 2), 2, 'el_GR'), ' €') AS gross_cost, 
    ...
    {
        $result['edit_url'] = $seoUrl->generate('administration/view_service.php', ['id' => $result['id']]);
    }
    ...
    if ($results !== false)
    {
        ...
        $response['columns'] = [
            ...
            ['field' => 'gross_cost', 'title' => TEXT_ITEMS_CLEAN_PRICE],
            ['field' => 'tax_factor', 'title' => TEXT_ITEMS_TAX],
        ...
        if (!$filters_initialized)
        {
            ...
            $tax = $db->query("SELECT 
                               tax_id AS value,  
                               CONCAT(ROUND(factor * 100, 2), '%') AS text  
                               FROM tax")->fetchAll();
